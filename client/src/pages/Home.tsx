...
function Header() {
  ...
  const nav = ["Tournaments", "Games", "Teams", "Clans", "Media"];
...
function LiveTicker() {
  return <div className="live-ticker"><div className="page-shell live-ticker__inner"><div className="live-ticker__label"><CircleDot size={13} /> Live now</div><div className="ticker-items">{liveMatches.map((match) => <div className="ticker-match" key={match.a}><span>{match.game}</span><strong>{match.a}</strong><b>{match.score}</b><strong>{match.b}</strong></div>)}</div><a href="#matches" className="ticker-link">View all <ArrowUpRight size={14} /></a></div></div>;
}

function TournamentSection() {
  return <section className="section-block" id="tournaments"><div className="page-shell"><div className="section-heading"><div><SectionLabel>01 / On the calendar</SectionLabel><h2>Find your next<br /><span>high-stakes room.</span></h2></div><a className="text-link" href="#all-tournaments">View all tournaments <ArrowUpRight size={15} /></a></div><div className="tournament-grid">{tournaments.map((tournament, index) => <article className={`tournament-card tournament-card--${index === 0 ? "featured" : ""}`} key={tournament.title} style={{ "--card-accent": tournament.accent } as React.CSSProperties}><div className="tournament-card__art" style={{ background: tournament.art }}><span className="card-index">0{index + 1}</span><span className="card-live-label">{index === 0 ? "REGISTRATION OPEN" : "OPEN FOR ENTRIES"}</span><div className="card-art-mark">{index === 0 ? <Shield size={50} strokeWidth={1} /> : <Trophy size={42} strokeWidth={1} />}</div></div><div className="tournament-card__body"><div className="card-meta"><span>{tournament.game}</span><span>{tournament.date}</span></div><h3>{tournament.title}</h3><p>{tournament.tag}</p><div className="tournament-card__footer"><span><small>Prize pool</small><strong>{tournament.prize}</strong></span><span><small>Capacity</small><strong>{tournament.teams}</strong></span><button className="circle-arrow" aria-label={`Open ${tournament.title}`}><ArrowUpRight size={16} /></button></div></div></article>)}</div></div></section>;
}

function LiveMatches() {
  return <section className="section-block section-block--tight" id="matches"><div className="page-shell"><div className="section-heading section-heading--row"><div><SectionLabel>02 / Watch the room</SectionLabel><h2>Live <span>matches.</span></h2></div><div className="live-status"><span className="pulse-dot" /> Scores update in real time</div></div><div className="match-list">{liveMatches.map((match) => <article className="match-row" key={match.a}><div className="match-row__game"><span className="game-mini game-mini--lime">{match.game.slice(0, 2).toUpperCase()}</span><span>{match.game}</span></div><div className="match-row__teams"><strong>{match.a}</strong><b>{match.score}</b><strong>{match.b}</strong></div><div className="match-row__progress"><div><span style={{ width: match.progress }} /></div><small>{match.status}</small></div><button className="watch-button"><Play size={13} fill="currentColor" /> Watch</button></article>)}</div></div></section>;
}
