...
const games = [
  { name: "Valorant", code: "V", events: "44 active events", tone: "from-lime-300/30 via-lime-300/5 to-transparent" },
  { name: "Mobile Legends", code: "M", events: "28 active events", tone: "from-fuchsia-300/30 via-fuchsia-300/5 to-transparent" },
  { name: "CS2", code: "C", events: "19 active events", tone: "from-sky-300/30 via-sky-300/5 to-transparent" },
  { name: "EA FC 26", code: "F", events: "12 active events", tone: "from-orange-300/30 via-orange-300/5 to-transparent" },
];
...
const media = [
  ["01", "THE LAST ROTATION", "Astra Forge close out the nightfall final", "from-lime-300/30 via-lime-900/20 to-black"],
  ["02", "CLUTCH THEORY", "Three clean rounds from the Orbit Syndicate", "from-violet-300/30 via-violet-900/20 to-black"],
  ["03", "RIVAL SIGNAL", "Inside the CS2 room before map three", "from-sky-300/30 via-sky-900/20 to-black"],
];
...
export default function Phase3Page() {
  ...
  return (
    <main className="min-h-screen bg-[#090a0b] pb-24 text-white">
      <header className="border-b border-white/10 bg-[#090a0b]/90 px-6 py-5 backdrop-blur md:px-12">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5">
          <Link href="/" className="font-display text-sm font-semibold tracking-[0.2em]">MENO<span className="text-lime-300">ARENA</span></Link>
      ...
      <section id="games" className="scroll-mt-10 border-y border-white/10 px-6 py-16 md:px-12"><div className="mx-auto max-w-7xl"><SectionLabel>04 / game hubs</SectionLabel><div className="mb-8 flex items-end justify-between"><h2 className="font-display text-4xl tracking-[-.04em] md:text-6xl">Pick your arena.</h2><Gamepad2 className="h-8 w-8 text-white/20" /></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{games.map(game => <Card key={game.name} className={`group overflow-hidden border-white/10 bg-gradient-to-br ${game.tone}`}><CardContent className="flex min-h-52 flex-col justify-between p-6"><div className="flex items-center justify-between"><span className="font-display text-4xl text-white/80">{game.code}</span><span className="font-mono text-[10px] text-white/40">[GAME_HEADER_IMAGE]</span></div><div><h3 className="font-display text-2xl">{game.name}</h3><p className="mt-1 text-xs text-white/45">{game.events}</p></div></CardContent></Card>)}</div></div></section>

      <section id="clans" className="scroll-mt-10 mx-auto grid max-w-7xl gap-10 px-6 py-16 md:px-12 lg:grid-cols-[.8fr_1.2fr]"><div><SectionLabel>05 / clan directory</SectionLabel><h2 className="font-display text-4xl tracking-[-.04em] md:text-6xl">Built by<br /><span className="text-lime-300">the best.</span></h2><p className="mt-6 max-w-sm text-sm leading-7 text-white/45">Find the organizations shaping the room. Rankings update as match results settle.</p><Button asChild variant="outline" className="mt-8 border-white/15 text-white hover:bg-white/10"><Link href="/dashboard/clan">Manage your clan <ChevronRight className="ml-2 h-4 w-4" /></Link></Button></div><Card className="border-white/10 bg-white/[.03]"><CardHeader className="flex-row items-center justify-between"><CardTitle className="font-display text-2xl">Global ranking</CardTitle><Trophy className="h-5 w-5 text-lime-300" /></CardHeader><CardContent className="space-y-1">{clans.map((clan, index) => <div key={clan[1]} className="grid grid-cols-[32px_1fr_90px_60px] items-center gap-3 border-t border-white/10 py-4 text-sm"><span className="font-mono text-xs text-white/35">0{index + 1}</span><span><strong className="block">{clan[0]}</strong><span className="font-mono text-[10px] text-white/35">{clan[1]} · GLOBAL</span></span><strong>{clan[2]}</strong><span className={clan[3].startsWith("+") ? "text-lime-300" : "text-red-300"}>{clan[3]}</span></div>)}</CardContent></Card></section>

      <section id="media" className="scroll-mt-10 border-t border-white/10 bg-white/[.02] px-6 py-16 md:px-12"><div className="mx-auto max-w-7xl"><div className="mb-8 flex items-end justify-between"><div><SectionLabel>06 / media gallery</SectionLabel><h2 className="font-display text-4xl tracking-[-.04em] md:text-6xl">Watch the room.</h2></div><ImageIcon className="h-8 w-8 text-white/20" /></div><div className="grid gap-4 md:grid-cols-3">{media.map(item => <Card key={item[0]} className={`group min-h-80 overflow-hidden border-white/10 bg-gradient-to-br ${item[3]} transition-transform duration-300 hover:-translate-y-1`}><CardContent className="flex h-full flex-col justify-between p-6"><div className="flex justify-between font-mono text-[10px] tracking-[.2em] text-white/45"><span>{item[0]}</span><Flame className="h-4 w-4 text-lime-300" /></div><div><p className="font-mono text-[10px] tracking-[.2em] text-lime-300">{item[1]}</p><h3 className="mt-2 font-display text-2xl leading-tight">{item[2]}</h3><Button variant="ghost" size="sm" className="mt-5 h-8 px-0 text-white/60 hover:bg-transparent hover:text-lime-300">Play highlight <ChevronRight className="ml-1 h-3 w-3" /></Button></div></CardContent></Card>)}</div></div></section>
